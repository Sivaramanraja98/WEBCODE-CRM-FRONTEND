...
const UpdateLead = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`https://crmbackend-6tdy.onrender.com/api/get-single-lead/${id}`).then(response => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.put(`https://crmbackend-6tdy.onrender.com/api/update-lead/${id}`, newEmployee);
            if (response) {
                setEmployeeDetails({
                    name: "",
                    email: "",
